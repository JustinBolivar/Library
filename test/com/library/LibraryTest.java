...
class LibraryTest {
    ...
    void testAddBook_CreateBook_ShouldReturnNotNullAndIsRentedIsFalse() {
        FictionBook manga = new FictionBook("ChainsawMan", "Fujimoto Tatsuki",
                2018);
    ...
    void testAddBooks_AddTwoBooksAndCheckIfLengthIsAlsoTwo_ShouldReturnTwo() {
        BookRentalSystem library = new BookRentalSystem();
        library.addBook(
                new FictionBook("ChainsawMan", "Fujimoto Tatsuki", 2018));
        library.addBook(
                new NonFictionBook("Atomic Habits", "James Clear", 2018));

        int expectedLength = 2;

        assertEquals(library.getBookLength(), expectedLength,
                () -> "The lenght of the library does not equal 4");
    }
    ...
    void testRentBookWithInvalidIndex_AddOneBooksAndRentInvalidIndex_ShouldReturnError() {
        ...
        assertThrows(IndexOutOfBoundsException.class,
                () -> library.rentBooks(1), "Should throw Index Out of Bounds");
    }
    ...
    void testAllDisplayBooks_AddTwoBooksAndCheckIfMatch_ShouldReturnTrue() {
        ...
        String expectedOutput = "Title: ChainsawMan, Author: Fujimoto Tatsuki, Year Published: 2018, Rented: false"
                + System.lineSeparator()
                + "Title: Atomic Habits, Author: James Clear, Year Published: 2018, Rented: false";

        assertEquals(expectedOutput, actualOutput,
                "The displayed book list does not match the expected output.");
    }
    ...
    void testAllDisplayRentedBooks_AddTwoBooksAndRentOneBookCheckIfItMatches_ShouldReturnTrue() {
        ...
        String expectedOutput = "Title: ChainsawMan, Author: Fujimoto Tatsuki, Year Published: 2018, Rented: true"
                + System.lineSeparator()
                + "Title: Atomic Habits, Author: James Clear, Year Published: 2018, Rented: false";
